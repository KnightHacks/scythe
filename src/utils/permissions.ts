...
export const RolePermissions = {
   ...
   */
  allNames(...roles: string[]): PermissionHandler {
   ...
   * @returns A permission handler function.
   */
  oneName(...roles: string[]): PermissionHandler {
    return async (interaction) => {
      // FIXME is this cast safe?
      const member = interaction.member as GuildMember;
   ...
   * @returns A permission handler function.
   */
  allIDs(...roleIDs: Snowflake[]): PermissionHandler {
    return async (interaction) => {
      const member = interaction.member as GuildMember;
      let allowed = true;
      if (!member) {
   ...
   * @returns A permission handler function.
   */
  oneID(...roleIDs: Snowflake[]): PermissionHandler {
    return async (interaction) => {
      const member = interaction.member as GuildMember;
      if (!member) {
        return false;
...
export const ChannelPermissions = {
   ...
   * @returns A permission handler function.
   */
  inNames(...channels: string[]): PermissionHandler {
    return async (interaction) => {
   ...
   * @returns A permission handler function.
   */
  inIDs(...channelIDs: Snowflake[]): PermissionHandler {
    return async (interaction) => {
...
export const CategoryPermissions = {
   ...
   */
  inNames(...categories: string[]): PermissionHandler {
    return async (interaction) => {
   ...
   * @returns A permission handler.
   */
  inIDs(...categoryIDs: Snowflake[]): PermissionHandler {
    return async (interaction) => {
      if (!(interaction.channel instanceof TextChannel)) {
        return false;
      }
